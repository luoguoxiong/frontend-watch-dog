...
class Catelog extends Component {
  ...
  clickTab({ id }, index) {
    const {
      actions: { fetchCatelogMsg, changeIdAndTabIndex },
    } = this.props;
    fetchCatelogMsg({ id });
  ...
  computedTabsProperty() {
    ...
    if (cateLogList.length > 0 && this.refDom) {
      ...
      tabsHeight =
        tabLen < clientHeight ? cateLogList.length * 42 : clientHeight;
      ...
      <Fragment>
        ...
        <div className="categogContet">
          <div
            ...
            style={{
              backgroundImage: `url(${activeCatalogMsg.wap_banner_url})`,
            }}
          >
          ...
          <div className="categoryTitle">
            <div></div>
            {activeCatalogMsg.name}分类<div></div>
          </div>
          <div className="subCategory">
            {"subCategoryList" in activeCatalogMsg
              ? activeCatalogMsg.subCategoryList.map((item) => (
                  <Link
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(Catelog)
);
